...
| ✍️ **Обработка текста локальным агентом** | Полноценная среда для написания текстов и заметок с минималистичным **Obsidian-подобным интерфейсом**. Боковая панель для быстрого создания и сортировки файлов в корне проекта. Документ слева, **чат с агентом справа** — выделите фрагмент и попросите переписать, его обработает **локальный агент без API-ключей**, ответ заменит выделенное. Плюс история правок, вкладки, **формулы (MathJax)**, закладки и панель форматирования. |
...
**Модуль обработки текста: чистый интерфейс в стиле Obsidian с AI-ассистентом**

![Обработка текста](assets/screenshots/textproc.png)

В последнем обновлении интерфейс модуля был полностью переработан для максимального комфорта при чтении и написании текстов:
- 🖋 **Абсолютно гладкие анимации** — появление боковой панели теперь происходит через плавный `width/opacity` переход, без резких скачков.
- 🗂 **Кнопки управления в стиле Obsidian** — добавлены постоянные, крупные и удобные кнопки быстрого доступа: «Новый файл», «Новая папка», «Реверс-сортировка (А-Я / Я-А)» и «Свернуть все папки».
- 📏 **Динамическая типографика** — появились настройки межстрочного интервала (1.0 / 0.8), которые автоматически и пропорционально масштабируют отступы между абзацами для идеального баланса текста.
- 🎨 **Премиальная эстетика** — убраны лишние надписи, фокусные элементы получили мягкое синее свечение вместо резкого желтого, а управление окном (светофор) стало аккуратнее.
